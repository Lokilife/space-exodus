...
using Content.Shared.Whitelist; // Exodus-ThickSyringe
...
public abstract class SharedInjectorSystem : EntitySystem
{
    ...
    [Dependency] private readonly EntityWhitelistSystem _whitelist = default!; // Exodus-ThickSyringe
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<InjectorComponent, BeforeInjectorUseEvent>(OnBeforeInjectorUse); // Exodus-ThickSyringe
    }

    // Exodus-ThickSyringe-Start | Mob whitelist check
    private void OnBeforeInjectorUse(Entity<InjectorComponent> injector, ref BeforeInjectorUseEvent ev)
    {
        if (ev.Cancelled)
            return;

        if (_whitelist.IsWhitelistFail(injector.Comp.MobWhitelist, ev.Target) ||
                _whitelist.IsBlacklistFail(injector.Comp.MobBlacklist, ev.Target))
        {
            _popup.PopupClient(
    ...
    {
        // Exodus-ThickSyringe-Start | Ability to cancel injection
        if (args.Handled || !args.CanReach || args.Target == null)
            return;

        var beforeEv = new BeforeInjectorUseEvent()
        {
            Injector = ent,
        ...
        args.Handled = TryUseInjector(ent, args.Target.Value, args.User);
        // Exodus-ThickSyringe-End
    }
    ...
    ("user", userName)), user, target);
        ...
        var doAfterStarted = _doAfter.TryStartDoAfter(new DoAfterArgs(EntityManager, user, actualDelay, new InjectorDoAfterEvent(), injector.Owner, target: target, used: injector.Owner)
        {
            BreakOnMove = true,
        ...
        if (doAfterStarted)
        {
            var ev = new InjectionStartedEvent()
            {
                Injector = injector,
                Target = target,
                ToggleMode = injector.Comp.ToggleState,
                User = user,
            };
        ...
        Entity<SolutionComponent> targetSolution, EntityUid user, bool asRefill)
    {
        ...
        AfterInject(injector, target, user); // Exodus-ThickSyringe
        return true;
    }
        ...
        Entity<SolutionComponent> targetSolution, EntityUid user)
    {
        ...
        AfterDraw(injector, target, user); // Exodus-ThickSyringe
        return true;
    }
    ...
    private void AfterInject(Entity<InjectorComponent> injector, EntityUid target, EntityUid user /* Exodus-ThickSyringe */)
    {
        var mode = injector.Comp.ToggleState; // Exodus-ThickSyringe
        ...
        // Exodus-ThickSyringe-Start
        var usedEv = new AfterInjectorUseEvent()
        {
            Injector = injector,
            Target = target,
            User = user,
    ...
    private void AfterDraw(Entity<InjectorComponent> injector, EntityUid target, EntityUid user /* Exodus-ThickSyringes */)
    {
        var mode = injector.Comp.ToggleState; // Exodus-ThickSyringe
