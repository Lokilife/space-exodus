...
public sealed partial class IngestionSystem : EntitySystem
{
    ...
    public static ProtoId<DamageTypePrototype> BeingEatenDamageType = "Blunt";
    ...
    private void OnEatingDoAfter(Entity<BodyComponent> entity, ref EatingDoAfterEvent args)
    {
        ...
        // Exodus-EatingMousesKillsThem-Start
        if (TryComp<MobThresholdsComponent>(food, out var mobThresholds) && _mobThreshold.TryGetDeadThreshold(food, out var deadThreshold, mobThresholds))
        {
            var damage = transfer * deadThreshold.Value * 2 / solution.Value.Comp.Solution.MaxVolume;
            _damageable.TryChangeDamage(entity, new(_proto.Index(BeingEatenDamageType), damage));
        }
        // Exodus-EatingMousesKillsThem-End
